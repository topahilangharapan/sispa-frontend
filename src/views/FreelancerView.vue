...
import type { FreelancerInterface } from '../interfaces/freelancer.interface.ts'
...
function filterData() {
  ...
  if (!query) {
    filteredData.value = [...freelancerStore.freelancers]
  } else {
    filteredData.value = freelancerStore.freelancers.filter(freelancer => {
...
function updateDisplayedData() {
  ...
  const dataSource = searchQuery.value ? filteredData.value : freelancerStore.freelancers
...
function refreshData() {
  ...
  freelancerStore.getFreelancers(token).finally(() => {
    searchQuery.value = '';
    currentPage.value = 1;
    filterData();
    updateDisplayedData();
...
onMounted(async () => {
  ...
  try {
    isLoading.value = true;
    const token = authStore.token ?? '';
    await freelancerStore.getFreelancers(token);

    // Initialize data for pagination
    filteredData.value = [...freelancerStore.freelancers];
    totalRows.value = freelancerStore.freelancers.length;
    updateDisplayedData();
...
<template>
  ...
  <div class="p-4 md:p-8 min-h-screen">
    <div class="max-w-full mx-auto mt-14">
      <!-- Page Header -->
      <div class="mb-6">
        <h1 class="text-2xl md:text-3xl font-bold text-black-grey-800">Freelancer Dashboard</h1>
      </div>
        ...
        <div class="p-5 border-b border-[#ECECEC] bg-gradient-to-r from-white to-[#FCFAFA]">
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center">
              ...
              <h2 class="text-xl font-bold text-[#2E2E2E]">Daftar Freelancer</h2>
            </div>
...
<!--                  </div>-->
                  <span class="text-center font-medium text-[#2E2E2E]">{{ freelancer.id }}</span>
                </div>
              </td>
        ...
        <div v-if="!isLoading && displayedFreelancers.length === 0" class="px-6 py-16 text-center">
          <div class="mx-auto max-w-md p-6 rounded-xl bg-[#F7F8FA] border border-dashed border-[#D8D8D8]">
            ...
            <p class="text-lg font-medium text-[#2E2E2E]">{{ searchQuery ? 'Tidak ada data freelancer yang sesuai' : 'Belum ada data freelancer' }}</p>
            <p class="mt-1 text-[#595959]">{{ searchQuery ? 'Coba gunakan kata kunci pencarian yang berbeda' : 'Belum ada data freelancer tersedia' }}</p>
