...
import type { FreelancerInterface } from '../interfaces/freelancer.interface'
...
const filteredData = ref<FreelancerInterface[]>([])
const displayedFreelancers = ref<FreelancerInterface[]>([])
...
function filterData() {
  ...
  if (!query) {
    filteredData.value = [...freelancerStore.freelancers as FreelancerInterface[]]
  } else {
    filteredData.value = (freelancerStore.freelancers as FreelancerInterface[]).filter(freelancer => {
      return freelancer.id?.toString().toLowerCase().includes(query) ||
        freelancer.name?.toLowerCase().includes(query) ||
        freelancer.phoneNumber?.toLowerCase().includes(query)
    })
  }
...
function updateDisplayedData() {
  ...
  const dataSource = searchQuery.value ? filteredData.value : (freelancerStore.freelancers as FreelancerInterface[])
  displayedFreelancers.value = dataSource.slice(start, end)
}

function refreshData() {
  isLoading.value = true;
  const token = authStore.token ?? '';
  freelancerStore.getApplicants(token).finally(() => {
    searchQuery.value = '';
    currentPage.value = 1;
    filterData();
    updateDisplayedData();
...
onMounted(async () => {
  ...
  try {
    ...
    filteredData.value = [...freelancerStore.freelancers as FreelancerInterface[]];
    totalRows.value = (freelancerStore.freelancers as FreelancerInterface[]).length;
    updateDisplayedData();
    isLoading.value = false;
  } catch (error) {
    console.error("Failed to fetch freelancer applicants:", error);
    isLoading.value = false;
  }
});
</script>
...
<template>
  ...
  <div class="p-4 md:p-8 min-h-screen">
    <div class="max-w-full mx-auto mt-14">
      <!-- Page Header -->
      <div class="mb-6">
        <h1 class="text-2xl md:text-3xl font-bold text-black-grey-800">Pendaftar Freelancer</h1>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-[#ECECEC] transition-all duration-300 hover:shadow-xl">
          ...
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center">
              <div class="p-2 bg-[#FEF6F6] rounded-lg mr-3">
                <User class="h-5 w-5 text-[#8F2527]" />
              </div>
              <h2 class="text-xl font-bold text-[#2E2E2E]">Daftar Pendaftar Freelancer</h2>
            </div>
            <div class="flex flex-col md:flex-row gap-3">
              ...
              <!-- View Freelancers Button -->
              <RouterLink :to="`/freelancer`">
                <VButton
                  variant="primary"
        ...
        <div v-else class="overflow-x-auto">
          <table id="freelancer-table" class="w-full">
            <thead>
              <tr class="bg-[#F7F8FA] text-[#2E2E2E]">
                <th class="px-6 py-4 text-center font-semibold">No.</th>
                <th class="px-6 py-4 text-left font-semibold">ID</th>
                <th class="px-6 py-4 text-left font-semibold">Nama</th>
                <th class="px-6 py-4 text-center font-semibold">No HP</th>
                <th class="px-6 py-4 text-center font-semibold">Status</th>
                <th class="px-6 py-4 text-center font-semibold">Aksi</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(freelancer, index) in displayedFreelancers" :key="freelancer.id"
                ...
                <td class="px-6 py-4 text-left">
                  <div class="flex items-center">
                    <span class="font-medium text-[#2E2E2E]">{{ freelancer.id }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 text-left font-medium text-[#2E2E2E]">
                  <div class="line-clamp-1 max-w-xs">{{ freelancer.name }}</div>
                </td>
                <td class="px-6 py-4 text-center text-[#595959]">{{ freelancer.phoneNumber }}</td>
                <td class="px-6 py-4 text-center">
                  <span
                    class="px-3 py-1 rounded-full text-sm font-semibold"
                    :class="freelancer.isWorking
        ...
        <!-- Empty State -->
        <div v-if="!isLoading && displayedFreelancers.length === 0" class="px-6 py-16 text-center">
          <div class="mx-auto max-w-md p-6 rounded-xl bg-[#F7F8FA] border border-dashed border-[#D8D8D8]">
            ...
            <p class="text-lg font-medium text-[#2E2E2E]">{{ searchQuery ? 'Tidak ada data pendaftar yang sesuai' : 'Belum ada data pendaftar' }}</p>
            <p class="mt-1 text-[#595959]">{{ searchQuery ? 'Coba gunakan kata kunci pencarian yang berbeda' : 'Belum ada data pendaftar tersedia' }}</p>
            <button v-if="searchQuery" @click="() => { searchQuery = ''; handleSearch({ target: { value: '' } } as unknown as Event); }"
