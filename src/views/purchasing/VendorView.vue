...
import type { VendorInterface } from '../../interfaces/vendor.interface.ts'
...
const filteredData = ref<VendorInterface[]>([])
const displayedVendors = ref<VendorInterface[]>([])
...
function filterData() {
  ...
  if (!query) {
    filteredData.value = [...vendorStore.vendors]
  } else {
    filteredData.value = vendorStore.vendors.filter(vendor => {
      return vendor.id?.toString().toLowerCase().includes(query) ||
...
function updateDisplayedData() {
  ...
  const dataSource = searchQuery.value ? filteredData.value : vendorStore.vendors
  displayedVendors.value = dataSource.slice(start, end)
}

function refreshData() {
  isLoading.value = true;
  const token = authStore.token ?? '';
  vendorStore.getVendors(token).finally(() => {
    searchQuery.value = '';
    currentPage.value = 1;
    filterData();
    updateDisplayedData();
...
onMounted(async () => {
  ...
  try {
    ...
    filteredData.value = [...vendorStore.vendors];
    totalRows.value = vendorStore.vendors.length;
    updateDisplayedData();
...
<template>
  ...
  <div class="p-4 md:p-8 min-h-screen">
    <div class="max-w-full mx-auto mt-14">
      <!-- Page Header -->
      <div class="mb-6">
        <h1 class="text-2xl md:text-3xl font-bold text-black-grey-800">Vendor Dashboard</h1>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-[#ECECEC] transition-all duration-300 hover:shadow-xl">
        ...
        <div class="p-5 border-b border-[#ECECEC] bg-gradient-to-r from-white to-[#FCFAFA]">
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center">
              ...
              <h2 class="text-xl font-bold text-[#2E2E2E]">Daftar Vendor</h2>
            </div>
        ...
        <div v-else class="overflow-x-auto">
          <table id="vendor-table" class="w-full">
            <thead>
            <tr class="bg-[#F7F8FA] text-[#2E2E2E]">
              <th class="px-6 py-4 text-center font-semibold">No.</th>
              <th class="px-6 py-4 text-left font-semibold">ID Vendor</th>
              <th class="px-6 py-4 text-center font-semibold">Nama</th>
              <th class="px-6 py-4 text-center font-semibold">Layanan</th>
              <th class="px-6 py-4 text-center font-semibold">Aksi</th>
            </tr>
            </thead>
            <tbody>
            <tr v-for="(vendor, index) in displayedVendors" :key="vendor.id"
              ...
              <td class="px-6 py-4 text-left">
                <div class="flex items-center">
                  ...
                  <span class="font-medium text-[#2E2E2E]">{{ vendor.id }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-left font-medium text-[#2E2E2E]">
                <div class="line-clamp-1 max-w-xs">{{ vendor.name }}</div>
              </td>
              <td class="px-6 py-4 text-center text-[#595959]">{{ vendor.service }}</td>
              <td class="px-6 py-4">
        ...
        <!-- Empty State -->
        <div v-if="!isLoading && displayedVendors.length === 0" class="px-6 py-16 text-center">
          <div class="mx-auto max-w-md p-6 rounded-xl bg-[#F7F8FA] border border-dashed border-[#D8D8D8]">
            ...
            <p class="text-lg font-medium text-[#2E2E2E]">{{ searchQuery ? 'Tidak ada data vendor yang sesuai' : 'Belum ada data vendor' }}</p>
            <p class="mt-1 text-[#595959]">{{ searchQuery ? 'Coba gunakan kata kunci pencarian yang berbeda' : 'Klik tombol "Tambah Vendor" untuk menambahkan vendor baru' }}</p>
            <RouterLink v-if="!searchQuery" :to="`/purchasing/vendor/add`">
              <VButton
                variant="primary"
