...
import { Search, Tag, User, Plus } from 'lucide-vue-next'
import type { ClientInterface } from '../../../interfaces/client.interface.ts'
...
const filteredData = ref<ClientInterface[]>([])
const displayedClients = ref<ClientInterface[]>([])
...
function filterData() {
  ...
  if (!query) {
    filteredData.value = [...clientStore.clients]
  } else {
    filteredData.value = clientStore.clients.filter(client => {
      return client.id?.toString().toLowerCase().includes(query) ||
...
function updateDisplayedData() {
  ...
  const dataSource = searchQuery.value ? filteredData.value : clientStore.clients
  displayedClients.value = dataSource.slice(start, end)
}

function refreshData() {
  isLoading.value = true;
  const token = authStore.token ?? '';
  clientStore.getClients(token).finally(() => {
    searchQuery.value = '';
    currentPage.value = 1;
    filterData();
    updateDisplayedData();
...
onMounted(async () => {
  ...
  try {
    isLoading.value = true;
    const token = authStore.token ?? '';
    await clientStore.getClients(token);

    // Initialize data for pagination
    filteredData.value = [...clientStore.clients];
    totalRows.value = clientStore.clients.length;
    updateDisplayedData();
...
<template>
  ...
  <div class="p-4 md:p-8 min-h-screen">
    <div class="max-w-full mx-auto mt-14">
      <!-- Page Header -->
      <div class="mb-6">
        <h1 class="text-2xl md:text-3xl font-bold text-black-grey-800">Klien Dashboard</h1>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-[#ECECEC] transition-all duration-300 hover:shadow-xl">
        ...
        <div class="p-5 border-b border-[#ECECEC] bg-gradient-to-r from-white to-[#FCFAFA]">
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center">
              ...
              <h2 class="text-xl font-bold text-[#2E2E2E]">Daftar Klien</h2>
            </div>
        ...
        <div v-else class="overflow-x-auto">
          <table id="client-table" class="w-full">
            ...
            <tr class="bg-[#F7F8FA] text-[#2E2E2E]">
              ...
              <th class="px-6 py-4 text-left font-semibold">ID Klien</th>
              <th class="px-6 py-4 text-center font-semibold">Nama</th>
              <th class="px-6 py-4 text-center font-semibold">Industri</th>
              <th class="px-6 py-4 text-center font-semibold">Aksi</th>
            </tr>
            </thead>
            <tbody>
            <tr v-for="(client, index) in displayedClients" :key="client.id"
              ...
              <td class="px-6 py-4 text-left">
                <div class="flex items-center">
                  ...
                  <span class="font-medium text-[#2E2E2E]">{{ client.id }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-left font-medium text-[#2E2E2E]">
                <div class="line-clamp-1 max-w-xs">{{ client.name }}</div>
              </td>
              <td class="px-6 py-4 text-center text-[#595959]">{{ client.industry }}</td>
              <td class="px-6 py-4">
        ...
        <!-- Empty State -->
        <div v-if="!isLoading && displayedClients.length === 0" class="px-6 py-16 text-center">
          <div class="mx-auto max-w-md p-6 rounded-xl bg-[#F7F8FA] border border-dashed border-[#D8D8D8]">
            ...
            <p class="text-lg font-medium text-[#2E2E2E]">{{ searchQuery ? 'Tidak ada data klien yang sesuai' : 'Belum ada data klien' }}</p>
            <p class="mt-1 text-[#595959]">{{ searchQuery ? 'Coba gunakan kata kunci pencarian yang berbeda' : 'Klik tombol "Tambah Klien" untuk menambahkan klien baru' }}</p>
            <RouterLink v-if="!searchQuery" :to="`/marketing/client/add`">
              <VButton
                variant="primary"
