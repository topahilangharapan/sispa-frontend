...
import { Search, FileText, Download, Trash2, Plus, ShoppingCart, FilePlus, Calendar, Tag, AlertCircle } from 'lucide-vue-next'
...
const filteredData = ref<PurchaseOrderInterface[]>([])
const displayedPurchaseOrders = ref<PurchaseOrderInterface[]>([])
...
// Watch for changes in purchase order data
...
const initializeTable = () => {
  totalRows.value = purchaseOrderStore.purchaseOrders.length
  updateDisplayedData()
}

const deletePurchaseOrder = async (id: number) => {
  try {
    await purchaseOrderStore.deletePurchaseOrder(id, authStore.token || '')
...
function filterData() {
  ...
  if (!query) {
    filteredData.value = [...purchaseOrderStore.purchaseOrders]
  } else {
    filteredData.value = purchaseOrderStore.purchaseOrders.filter(order => {
...
function refreshData() {
  if (authStore.token) {
    isLoading.value = true
    purchaseOrderStore.fetchAll(authStore.token).finally(() => {
      isLoading.value = false
...
function updateDisplayedData() {
  ...
  const dataSource = searchQuery.value ? filteredData.value : purchaseOrderStore.purchaseOrders

  displayedPurchaseOrders.value = dataSource.slice(start, end)
}
</script>
      ...
      <div class="bg-white rounded-xl shadow-lg overflow-hidden border border-[#ECECEC] transition-all duration-300 hover:shadow-xl">
          ...
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center">
              <div class="p-2 bg-[#FEF6F6] rounded-lg mr-3">
                <ShoppingCart class="h-5 w-5 text-[#8F2527]" />
              </div>
              <h2 class="text-xl font-bold text-[#2E2E2E]">Daftar Purchase Order</h2>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-3">
              ...
              <!-- Add Button -->
              <VButton @click="() => router.push('/marketing/purchase-order/create')"
                       variant="primary"
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody>
            <tr v-for="(order, index) in displayedPurchaseOrders" :key="order.id"
              ...
              <td class="px-6 py-4 text-left">
                <div class="flex items-center">
                  ...
                  <span class="font-medium text-[#2E2E2E]">{{ order.id }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-left font-medium text-[#2E2E2E]">
                <div class="line-clamp-1 max-w-xs">{{ order.companyName }}</div>
              </td>
              <td class="px-6 py-4 text-left">
                <div class="flex items-center">
                  ...
                  <span class="text-[#595959]">{{ formatDate(order.dateCreated) }}</span>
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center justify-center space-x-2">
                  <!-- Detail Button -->
                  <button @click="goToDetail(order.id)"
                  ...
                  <!-- Download Button -->
                  <button @click="downloadPurchaseOrder(order.id, authStore.token || '')"
                  ...
                  <!-- Delete Button -->
                  <button @click="() => (showDialog = order.id)"
            ...
            <!-- Empty State -->
            <tr v-if="displayedPurchaseOrders.length === 0">
              <td colspan="5" class="px-6 py-16 text-center">
                <div class="mx-auto max-w-md p-6 rounded-xl bg-[#F7F8FA] border border-dashed border-[#D8D8D8]">
                  ...
                  <p class="text-lg font-medium text-[#2E2E2E]">{{ searchQuery ? 'Tidak ada data purchase order yang sesuai' : 'Belum ada data purchase order' }}</p>
                  <p class="mt-1 text-[#595959]">{{ searchQuery ? 'Coba gunakan kata kunci pencarian yang berbeda' : 'Klik tombol "Tambah Purchase Order" untuk membuat purchase order baru' }}</p>
                  <VButton v-if="!searchQuery" @click="() => router.push('/marketing/purchase-order/create')"
                           variant="primary"
